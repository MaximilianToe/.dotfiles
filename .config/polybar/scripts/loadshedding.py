...
def get_current_time():
    return datetime.datetime.now(pytz.timezone("Germany/Berlin"))
...
def find_next_event(events):
    ...
    if (now >= start) and (now <= end):
        ...
        print("%{F#db4b4b} %{F#e0af68}󱎫 %{F-}" + f"{hours:02d}:{minutes:02d}" + " %{F#} ")
...
def main():
    try:
        with open(
            "/home/max/.config/polybar/scripts/loadshedding.json", "r"
        ) as file:
            # Load data.
